...
class MockDataSource(DataSource):
    ...
    def __init__(
        self,
        name: str,
        source_type: DataSourceType = DataSourceType.FUND,
    ...
    async def fetch(self, *args, **kwargs) -> DataSourceResult:
        ...
        if self.should_fail:
            return DataSourceResult(
                success=False, error=f"Mock failure for {self.name}", timestamp=0, source=self.name
            )
        return DataSourceResult(
            success=True,
...
class TestDataSourceConfig:
    ...
    def test_default_values(self):
        """测试默认值"""
        config = DataSourceConfig(
            source_class=MockDataSource, name="test", source_type=DataSourceType.FUND
        )
