...
class FundResponse(BaseModel):
    ...
    estimateChangePercent: Optional[float] = Field(None, alias="estimated_growth_rate", description="估算增长率(%)")
    estimateTime: Optional[str] = Field(None, alias="estimate_time", description="估值时间")
...
class CommodityResponse(BaseModel):
    """商品响应模型"""
    commodity: Optional[str] = Field(None, description="商品类型")
    symbol: str = Field(..., description="交易符号")
    name: str = Field(..., description="商品名称")
    price: float = Field(..., description="当前价格")
    change: Optional[float] = Field(None, description="涨跌额")
    changePercent: Optional[float] = Field(None, alias="change_percent", description="涨跌幅(%)")
    currency: Optional[str] = Field(None, description="货币")
    exchange: Optional[str] = Field(None, description="交易所")
    timestamp: Optional[str] = Field(None, alias="time", description="更新时间")
    source: str = Field(..., description="数据源")
    ...
    prevClose: Optional[float] = Field(None, alias="prev_close", description="昨收价")
...
class DataSourceHealthItem(BaseModel):
    ...
    response_time_ms: Optional[float] = Field(None, description="响应时间(ms)")
